document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchTodayStats = async () => {
        try {
            ...
            // Update todayMain card
            if (todayMain) {
                const h1 = todayMain.querySelector('h1');
                const h3 = todayMain.querySelector('h3');
                
                if (h1 && data.calories !== undefined) {
                    h1.classList.remove('skeleton'); 
                    h1.style.minWidth = '0';
                    const targetVal = Math.round(data.calories);
                    // Animate from 0 to target
                    animateValue(h1, 0, targetVal, 1000);
                }
                if (h3) {
                    h3.textContent = 'Calories Consumed';
                }
            }
        } catch (error) {
    ...
    const playStatsAnimation = async () => {
        ...
        const apiUrl = activeTab === 'monthly' ? '/api/monthlyStats' : '/api/weeklyStats';
    ...
    const renderFoodList = (foods) => {
        ...
        foods.forEach(food => {
            ...
            div.addEventListener('click', () => {
                ...
                // Store full nutrients for display (Cal, Pro, Carb, Fat, Fib)
                const nutrientArray = [food.macros.cal, food.macros.p, food.macros.c, food.macros.f, food.macros.fib];
                selectedFoodInput.setAttribute('data-nutrients', JSON.stringify(nutrientArray));
                
                foodSearch.value = food.name;
                updateNutritionDisplay(nutrientArray);
    ...
    // Profile Elements
    const profileItem = document.querySelector('.menuItem span.menuIcon:contains("👤")').parentElement; // Selecting by icon content safely if ID missing, or let's just add ID in template?
    // Actually, safer to add ID to the menu item in HTML. But I can't edit HTML again just for ID efficiently if I can select it. 
    // The "Profile" item is the first one.
